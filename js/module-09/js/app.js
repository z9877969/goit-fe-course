...
const createNewNote = (event) => {
  ...
  event.preventDefault();
  addListItem(noteListLink, createListItem(...notepad.notes.slice(-1)));
...
const search = () => {
  [...noteListLink.children].forEach(item => item.remove());
  renderNoteList(noteListLink, notepad.filterNotesByQuery(refs.searchInput.value))
}
...
noteListLink.addEventListener('click', removeListItem);
